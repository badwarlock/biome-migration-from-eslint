...
> ⚠️ **Важно:** Эта миграция использует **Biome 2.0** с новым синтаксисом `files.includes` (вместо устаревших `files.include` и `files.ignore` из Biome 1.x)

## 🎯 Версия Biome

Данная конфигурация рассчитана на **Biome 2.0+**.

**Основные изменения в Biome 2.0:**
- ❌ **Удалено**: `files.include` и `files.ignore`
- ✅ **Добавлено**: `files.includes` с новым glob-синтаксисом
- 🔄 Паттерны с `!` для исключений (ex: `!node_modules/**`)
- 📝 Новая семантика: `*` ≠ `**/*` (раньше были эквивалентны)

**Миграция из Biome 1.x:**
Если у вас есть старая конфигурация Biome 1.x, запустите:
```bash
npx biome migrate
```
